let allAgents = [
    ...
    },
    {
      ...
      {
        name: '300-AutoPorX-816-281-6544',
        id: 'u020-autoporx--bnvs',
        url: 'https://dialogflow.cloud.google.com/#/editAgent/u020-autoporx--bnvs/'
      },
      {
...
// get names from allAgents array
let allAgentsNames = allAgents.map(agent => agent.name);

//check if agent is in the list of agents to change
function checkAgent(agentName) {
  for (let i = 0; i < changeAgents1.length; i++) {
    if (agentName === changeAgents1[i]) {
      return true;
    }
  }
  return false;
...
// get values from array 1 not in array 2
function getValuesNotInArray1(array1, array2) {
    let result = [];
...
setTimeout(() => {
    console.log(getValuesNotInArray1(changeAgents1, allAgentsNames));
}, 3000);