...
let timeRegex = '8:00 AM';
let vehicleRegex = '2015 Carrera';
let array = [
    [{
        Query: 'Hi',
    },
...
async function run(phoneNumber, queries){
    ...
    for(let el = 1; el < 50; el ++){
        ...
        for (let index = 0; index < queries.length; index++) {
            ...
            console.log(g);
            if(res.toLowerCase().includes('availability')){
                let timeRegex23 = rsp.match(/\d{1,2}:\d{2} (?:AM|PM)/g);
                timeRegex = timeRegex23[0]
...
async function processArray1(array) {
    for(const queries of array) {
        let phoneNumber = queries.customerPhone;
        await run(phoneNumber, queries)
    }
    console.log('Done Inside!');
}
async function processArray2(array) {
for (const item of array) {
    await processArray1(item);
}
console.log('Done Outside!');
}

async function main(){
    await processArray2(array);
    await pushData();
}
main();
// exports.RunTests = async (array) => {
//     await processArray2(array);
