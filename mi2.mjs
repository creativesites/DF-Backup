...
async function getSingleLogs(startDate, agent){
    ...
    async function getC(){
      ...
      let isDate = moment(date1).isSame(dt1)
      let before = moment(date1).isBefore(dt1);
...
app.post('/singleLogs', (req, res) => {
    
    const data = req.body;
    //console.log(req)
    let dtR = data.startDate;
    let dtF = moment(dtR).format('YYYY-MM-DD')
    let startDate = dtF;
    let agent = data.agent;
    console.log(startDate)
    console.log(agent)
    res.status(200).send(`getting logs for ${agent} on ${startDate}`);
    getSingleLogs(startDate, agent)
});

app.listen(PORT, () => console.log(`backend running on port ${PORT}`))

getSingleLogs('2022-10-13', '300-AutoPorsBell-8162816544')