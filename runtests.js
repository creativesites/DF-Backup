...
const queries = [
    ...
    },
    {
        ...
        "Query": "Yes",
        "ExpectedResponse": [
            "Now, please tell me what services you would like?",
    ...
    },
    {
        ...
        "Query": "car wash",
        "ExpectedResponse": [
            "Would you like to drop off your vehicle, wait at the dealership, or would you need a ride?",
            "drop off your vehicle, wait at the dealership"
        ],
        "CaseType": "BookingTSS-S",
        "Test": "Test1-1-3"
    },
    {
        ...
        "Query": "Drop off.",
        "ExpectedResponse": [
            "Our first availability",
            "Got it. What day and time would you like to bring your car?",
            "availability",
            "day and time"
        ],
        "CaseType": "BookingTSS-S",
...
async function run(phoneNumber, queries){
    ...
    await doc.loadInfo();
    const sheet = doc.sheetsByTitle['TestsSheet1'];
    ...
    const url1 = 'https://dialogflow.cloud.google.com/#/agent/j0v-davidg-idno/editIntent/726888fe-ea20-4fc4-ac7e-015fbff7399e/';
    ...
    try {
        await page1.waitForSelector("aria/Enter value")
        await page1.click("aria/Enter value")
        await page1.waitForTimeout(1000);
    } catch (error) {
        console.log('err selecting textfield')
        await page1.waiitForSelector("#intent-parameters-table > tbody > tr:nth-child(1) > td:nth-child(5) > div > textarea")
        await page1.click("#intent-parameters-table > tbody > tr:nth-child(1) > td:nth-child(5) > div > textarea")
        await page1.waitForTimeout(1000);
    }
    try {
        const input = await page1.$('aria/Enter value');
        await input.click({ clickCount: 3 })
        await page1.waitForTimeout(1000);
        await input.type(phoneNumber);
    } catch (error) {
        const inputValue = await page1.$eval('aria/Enter value', el => el.value);
        ...
        await page1.type('aria/Enter value', phoneNumber);
    }
    await page1.waitForTimeout(1000);
    ...
    for (let index = 0; index < queries.length; index++) {
        ...
        let q = element.Query
        // type query
        await page1.waitForSelector('aria/Try it now')
        await page1.click('aria/Try it now')
        let testtInput = await page1.$('aria/Try it now')
        await testtInput.type(q);
        await page1.waitForTimeout(1000);
        await page1.keyboard.press('Enter');
        await page1.waitForTimeout(8000);
        // get response
        await page1.waitForSelector('#test-console > div.side-box > div.animate-show.short > div:nth-child(5) > console-response-content > div.side-row > div.side-row > div > div > div > div > span > span')
        let resEl = await page1.$('#test-console > div.side-box > div.animate-show.short > div:nth-child(5) > console-response-content > div.side-row > div.side-row > div > div > div > div > span > span')
        // get text content from element
        let res = await page1.evaluate(el => el.textContent, resEl);
        res = res.replace(/(<([^>]+)>)/gi, "");
        console.log(res)
        // get intent
        await page1.waitForSelector('#test-client-link-intent')
        let intel = await page1.$('#test-client-link-intent')
        let intent = await page1.evaluate(el => el.textContent, intel);
        console.log(intent)
        // get contexts
        let contexts = ''
        try {
            let contextsEls = await page1.$$('.token-label')
            contexts = await page1.$$eval(
                '.token-label',
        ...
        let expRes = element.ExpectedResponse
        let expResString = expRes.toString();
        let sendResp = {
            "SeqNum": element.seqNum,
            "Date": moment().format('MM/DD/YYYY'),
            "UserInput": q,
            "Response": res,
            "ExpectedResponses": expResString,
            "PassOrFail": "",
            "IntentMatchedName": intent,
            "Agent": "00-000-DEMO1-POR-8184921287",
            "Test": element.Test,
            "CaseType": element.CaseType,
        }
        let g = expRes.some(r=> res.includes(r));
        console.log(g);
        if(g == true){
            sendResp.PassOrFail = 'Pass';
        }else{
            sendResp.PassOrFail = 'Fail';
            
        }
        await sheet.addRow(sendResp);
