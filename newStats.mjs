...
async function runInContext(){
    ...
    const url1 = 'https://dialogflow.cloud.google.com';
        ...
        // loop over changeAgents1
        for (var i = 0; i < changeAgents1.length; i++) {
            let el = changeAgents1[i];
            await doc.useServiceAccountAuth({
              ...
              await doc.loadInfo();
              const sheet = doc.sheetsByTitle['TempLogs'];
              ...
              await page1.waitForTimeout(2000);
              try {
                console.log('selecting select dates button')
                await page1.waitForXPath('/html/body/div[1]/div[2]/div/div/div/section/div/div[3]/div/history/div/div[2]/div/div[1]/div[2]/md-datepicker[1]/div/button')
                let butt = await page1.$x('/html/body/div[1]/div[2]/div/div/div/section/div/div[3]/div/history/div/div[2]/div/div[1]/div[2]/md-datepicker[1]/div/button')
                await page1.waitForTimeout(1000);
                await butt[0].click();
                 await page1.waitForSelector('aria/September 2022')
                await page1.click('aria/September 2022')
                await page1.waitForTimeout(1000);
                await page1.waitForSelector('aria/Monday September 19 2022')
                await page1.click('aria/Monday September 19 2022')
                await page1.waitForTimeout(20000);
            } catch (error) {
                console.log(error);
            }
              // loop through iid from 1
              let adjustedStart = 1
              if (i === 0) {
                adjustedStart = 45
              }
              ...
              async function goGet(){
                for(var idx = adjustedStart; idx < iid; idx++) {
                    await page1.waitForTimeout(5000);
                    let arrVal 
                    async function inside(){
                        try{
                            ...
                            // get convestation type and number of iterations 
                            try {
                                let numberOfIterationsForChat = await page1.$(`#main > div > div.workplace.ng-scope > div > history > div > div.content-section.ng-scope > conversations > div > div:nth-child(${arrVal}) > div > div.layout-align-start-center.layout-row.flex-85 > span.size.ng-binding`)
                                let itNum = await page1.evaluate(element => element.textContent, numberOfIterationsForChat);
                                let chaT = await page1.$(`#main > div > div.workplace.ng-scope > div > history > div > div.content-section.ng-scope > conversations > div > div:nth-child(${arrVal}) > div > div.layout-align-start-center.layout-row.flex-85 > span.name.ng-binding`)
                                let chatType = await page1.evaluate(element => element.textContent, chaT);
                                let onh = {}
                                ...
                                itNum = itNum.replace(/\D/g,'')
                                onh.CallDate = startTime;
                                onh.Iterations = parseInt(itNum);
                                onh.Agent = agent;
                                onh.ConversationType = chatType;
                                tempArr.push(onh);
                            } catch (error) {
                    ...
                    // click next page based on arrVal
                    let modNum = idx % 100
                    // if modulus is 1
                    console.log(`now at index: ${idx}, arrVal: ${arrVal}, modNum: ${modNum}`)
                    if(idx < 100){
                        arrVal = idx; 
                        if(idx === 0){
                            // click 100 chats per page
                        ...
                        if(modNum === 1){
                            ...
                            console.log(`next at index: ${idx}, arrVal: ${arrVal}`)
                        }
                        await inside()
                    }
                    else{
                        let sgh = idx.toString()
                        let bcn = sgh.substring(sgh.length - 2)
                        let aw = parseInt(bcn)
                        if(aw === 0){
                            arrVal = 100
                            if(modNum === 1){
                    ...
                    // get all data from page1 
                    
    
                  }
              }
              await goGet().then(async()=>{
                try {
                    await sheet.addRows(tempArr) 
                    tempArr = [];
                }catch(err){
                    console.log(onh)
                }
